...
func (r *Local) Fetch(ctx context.Context) error {
	...
	for _, remote := range remotes {
		...
		for _, url := range remote.Config().URLs {
			...
			authMethod, err = auth.NewSSHAuthMethod(*endpoint)
			if err != nil {
				slog.Error("failed to detect SSH auth method", "error", err)
				continue
			}
			ssh.DefaultAuthBuilder = func(_ string) (ssh.AuthMethod, error) {
...
func (c *Client) Sync(ctx context.Context, filter string, prune bool) error {
	...
	for _, r := range repos {
		...
		if err := r.Fetch(ctx); err != nil {
			syncErrors = append(syncErrors, fmt.Errorf("%s: fetch failed: %w", r.Name(), err))
			continue
		}
		...
		defaultBranch, err := r.DefaultBranch()
		if err != nil {
			syncErrors = append(syncErrors, fmt.Errorf("%s: failed to get default branch: %w", r.Name(), err))
			continue
		}
		...
		currentBranch, err := r.Branch()
		if err != nil {
			syncErrors = append(syncErrors, fmt.Errorf("%s: failed to get current branch: %w", r.Name(), err))
			continue
		}

		if currentBranch != defaultBranch {
			if err := r.CheckoutBranch(defaultBranch); err != nil {
				syncErrors = append(syncErrors, fmt.Errorf("%s: checkout failed: %w", r.Name(), err))
				continue
			}
			defer func() {
				if err := r.CheckoutBranch(currentBranch); err != nil {
					slog.Error("failed to checkout original branch", "error", err, "repo", r.Name())
				}
			}()
		}
		...
		if err := r.Pull(ctx); err != nil {
			syncErrors = append(syncErrors, fmt.Errorf("%s: pull failed: %w", r.Name(), err))
			continue
		}
	}

	if len(syncErrors) > 0 {
		return fmt.Errorf("%d repositories failed to sync: %w", len(syncErrors), errors.Join(syncErrors...))
	}
